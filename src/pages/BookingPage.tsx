...
import type { Performance, Reservation } from '../types';
...
export default function BookingPage() {
  ...
  const [step, setStep] = useState<'reserve' | 'payment' | 'complete'>('reserve');
  ...
  const handleReservation = async () => {
    try {
      ...
      // Step 2: Create reservation (임시 예약)
      const reservationResponse = await apiService.createReservation(performance.id, seatCount);
      ...
      setError(
        axiosErr.response?.data?.message || (err as Error).message || '예약에 실패했습니다.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (paymentResponse.finalStatus === 'SUCCESS') {
        // Confirm reservation
        await apiService.confirmReservation(performance.id, reservation.reservationId);
      ...
      const errorMsg =
        axiosErr.response?.data?.message || (err as Error).message || '결제에 실패했습니다.';
      setError(errorMsg);

      // If payment failed, cancel reservation
      if (reservation) {
        try {
          await apiService.cancelReservation(performance.id, reservation.reservationId);
        } catch (cancelErr) {
  ...
  return (
    <div className='booking-page'>
      <button className='btn-back' onClick={() => navigate(`/performances/${id}`)}>
        ← 뒤로가기
      </button>
      ...
      <div className='booking-container'>
        ...
        <div className='progress-steps'>
          <div className={`step ${step === 'reserve' ? 'active' : 'completed'}`}>
          ...
          <div
            className={`step ${
              step === 'payment' ? 'active' : step === 'complete' ? 'completed' : ''
            }`}>
        ...
        <div className='booking-summary'>
          ...
          <div className='summary-content'>
            ...
            <div className='summary-row'>
              ...
              <span className='summary-value'>{performance.price.toLocaleString()}원</span>
            </div>
            <div className='summary-row total'>
              <span className='summary-label'>총 결제 금액</span>
              <span className='summary-value total-price'>{totalPrice.toLocaleString()}원</span>
            </div>
          </div>
        </div>
          ...
          <div className='step-content'>
            ...
            <div className='reservation-info'>
              <div className='info-badge success'>
                ✓ 예약이 완료되었습니다 (예약번호: {reservation.reservationId})
              </div>
              <p className='reservation-notice'>
                10분 이내에 결제를 완료해주세요. 시간 초과 시 예약이 자동으로 취소됩니다.
              </p>
            </div>
        ...
        {step === 'complete' && (
          <div className='step-content complete'>
            ...
            <div className='complete-actions'>
              ...
              <button className='btn btn-secondary' onClick={() => navigate('/performances')}>
                공연 목록으로
              </button>
            </div>
          </div>
