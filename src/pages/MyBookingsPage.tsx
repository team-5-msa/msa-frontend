...
export default function MyBookingsPage() {
  ...
  const loadBookings = async () => {
    try {
      ...
      const sorted = data.sort((a, b) => b.createdAt._seconds - a.createdAt._seconds);
      setBookings(sorted);
    } catch (err) {
      setError((err as Error).message || '예매 내역을 불러오는데 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCancelBooking = async (bookingId: string) => {
    ...
    try {
      ...
      alert(
        axiosErr.response?.data?.message || (err as Error).message || '예매 취소에 실패했습니다.'
      );
    } finally {
  ...
  const getStatusBadge = (status: string) => {
    const badges: Record<string, { label: string; className: string }> = {
      pending: { label: '결제 대기', className: 'status-pending' },
      paid: { label: '결제 완료', className: 'status-paid' },
      CANCELLED: { label: '취소됨', className: 'status-cancelled' },
      payment_failed: { label: '결제 실패', className: 'status-failed' },
    };
            ...
            return (
              <div key={booking.bookingId} className='booking-card'>
                <div className='booking-header'>
                  <div className='booking-id'>예매번호: {booking.bookingId}</div>
                  <div className={`booking-status ${status.className}`}>{status.label}</div>
                </div>
                ...
                <div className='booking-content'>
                  <div className='booking-info'>
                    <div className='info-row'>
                      ...
                      <div>
                        ...
                        <div className='info-value'>{booking.performanceId}</div>
                      </div>
                    </div>
                    ...
                    {booking.seatIds && booking.seatIds.length > 0 && (
                      <div className='info-row'>
                        ...
                        <div>
                          ...
                          <div className='info-value'>{booking.seatIds.join(', ')}</div>
                        </div>
                      </div>
                    )}
                    ...
                    {booking.reservationId && (
                      <div className='info-row'>
                        ...
                        <div>
                          ...
                          <div className='info-value'>{booking.reservationId}</div>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className='booking-footer'>
                    <div className='booking-date'>
                      ...
                      <div className='date-value'>{formatDate(booking.createdAt)}</div>
                    </div>
